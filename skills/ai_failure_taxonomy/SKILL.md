...
## Purpose
- Classify agent failures so fixes target the real weakness instead of symptoms.
...
## Use when
- Use after bad runs, hallucinations, weak outputs, or missed tool use.
- The main bottleneck is best solved through ai work rather than generic brainstorming.
- There is enough context to act, or the first useful move is to identify what is missing.

## Avoid when
- Do not use when there is no concrete failure evidence to classify.
...
## Example invocation
- Slash: `/ai_failure_taxonomy <task>`
- Natural language: "Use aI Failure Taxonomy to classify agent failures so fixes target the real weakness instead of symptoms."
